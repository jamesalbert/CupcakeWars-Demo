...
function message_box(self, x, y)
    self = {}
    self.x = x
    self.y = y
    self.image = love.graphics.newImage( "message.png" )
...
function handle_jump_event()
    if cupcake.y <= cupcake.last_y - cupcake.jump then
        cupcake.yspeed = 15
    elseif cupcake.y == cupcake.last_y and cupcake.yspeed == 15 then
        cupcake.yspeed = 0
    end
end
...
function set_level_borders()
    ...
    front = close_oven(front, 1200, 0, (screen.w/2), screen.h)
end

function message_box_switch()
    if cupcake.x >= oven.width/5 then
        love.graphics.draw(message.image, message.x, message.y)
        love.graphics.print("Thank you for playing\n\t\tCupcake Wars\t\t", message.x+((message.w/2)-50), message.y+((message.h/2)-50), 0, 1, 1)
    end
end
...
function love.load()
    ...
    message = message_box(message, 800, 100)
    load_collisions()
    make_cupcakes()
end
